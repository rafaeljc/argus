...
class SuspendUserTest {
    ...
    void setUp() {
        ...
        useCase = new SuspendUser(userLifecycle, auditLogRepository, clock);
    }
    ...
    @Test
    void suspend_stateChanged_insertsAuditRowWithReason() {
        UserId targetId = newUserId();
        UserId actorId = newUserId();
        User suspended = user(targetId, true);
        when(userLifecycle.suspend(targetId)).thenReturn(new UserStateChange(suspended, true));

        User result = useCase.suspend(targetId, actorId, "abuse report");
        ...
        assertThat(entry.metadata()).isEqualTo(new AuditMetadata.UserAction("abuse report"));
        assertThat(entry.createdAt()).isEqualTo(NOW);
    }

    @Test
    void suspend_noOp_doesNotInsertAuditRow() {
        UserId targetId = newUserId();
        UserId actorId = newUserId();
        User alreadySuspended = user(targetId, true);
        when(userLifecycle.suspend(targetId)).thenReturn(new UserStateChange(alreadySuspended, false));

        User result = useCase.suspend(targetId, actorId, "abuse report");

        assertThat(result).isSameAs(alreadySuspended);
